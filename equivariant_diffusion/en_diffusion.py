...
def gaussian_KL_for_dimension(q_mu, q_sigma, p_mu, p_sigma, d):
    ...
    return (
        d * torch.log(p_sigma / (q_sigma + 1e-8) + 1e-8)
...
class EnHierarchicalVAE(torch.nn.Module):
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context):
        ...
        # Encoder output.
        z_x_mu, z_x_sigma, z_h_mu, z_h_sigma = self.encode(
        ...
        assert (
            z_x_sigma.mean(dim=(1, 2), keepdim=True)
            .expand_as(z_x_sigma)
            .allclose(z_x_sigma, atol=1e-7)
        )
        zeros, ones = (
            torch.zeros_like(z_x_mu),
            torch.ones_like(z_x_sigma.mean(dim=(1, 2))),
        )
        subspace_d = self.subspace_dimensionality(node_mask)
    ...
    def encode(self, x, h, node_mask=None, edge_mask=None, context=None):
        ...
        # Encoder output.
        z_x_mu, z_x_sigma, z_h_mu, z_h_sigma = self.encoder._forward(
            xh, node_mask, edge_mask, context
        )
    ...
    def decode(self, z_xh, node_mask=None, edge_mask=None, context=None):
        ...
        h_cat = xh[
            :, :, self.n_dims : -1
        ]  # TODO: have issue when include_charges is False
        h_cat = F.one_hot(torch.argmax(h_cat, dim=2), self.num_classes) * node_mask
        h_int = torch.round(h_int).long() * node_mask
        h = {"integer": h_int, "categorical": h_cat}
...
class EnLatentDiffusion(EnVariationalDiffusion):
    ...
    def log_pxh_given_z0_without_constants(
        ...
        # the weighting in the epsilon parametrization is exactly '1'.
        log_pxh_given_z_without_constants = -0.5 * self.compute_error(
            net_out, gamma_0, eps
        )
