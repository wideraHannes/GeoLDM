...
from src.geoldm.qm9.utils import compute_mean_mad
from src.geoldm.qm9.sampling import sample
from src.geoldm.qm9.property_prediction.main_qm9_prop import test
from src.geoldm.qm9.property_prediction import main_qm9_prop
from src.geoldm.qm9.sampling import sample_sweep_conditional
...
def get_classifier(dir_path="", device="cpu"):
    ...
    classifier_state_dict = torch.load(
        join(dir_path, "best_checkpoint.npy"), map_location=torch.device("cpu")
    )
...
def get_generator(dir_path, dataloaders, device, args_gen, property_norms):
    ...
    model, nodes_dist, prop_dist = get_latent_diffusion(
        args_gen, device, dataset_info, dataloaders["train"]
    )
...
class DiffusionDataloader:
    ...
    def sample(self):
        ...
        if self.unkown_labels:
            ...
            context = (
                context * self.prop_dist.normalizer[prop_key]["mad"]
                + self.prop_dist.normalizer[prop_key]["mean"]
            )
        data = {
            "positions": x.detach(),
...
def main_quantitative(args):
    ...
    model, nodes_dist, prop_dist, _ = get_generator(
        args.generators_path, dataloaders, args.device, args_gen, property_norms
    )
    ...
    if args.task == "edm":
        diffusion_dataloader = DiffusionDataloader(
            args_gen,
            model,
            nodes_dist,
            prop_dist,
        ...
        dataloaders["train"].dataset.data[args.property] = dataloaders["train"].dataset.data[
            args.property
        ][idxs]
        loss = test(
            classifier,
...
def save_and_sample_conditional(args, device, model, prop_dist, dataset_info, epoch=0, id_from=0):
    ...
    vis.save_xyz_file(
        "outputs/%s/analysis/run%s/" % (args.exp_name, epoch),
        one_hot,
        charges,
        x,
        dataset_info,
...
def main_qualitative(args):
    ...
    dataloaders = get_dataloader(args_gen)
    property_norms = compute_mean_mad(dataloaders, args_gen.conditioning, args_gen.dataset)
    model, nodes_dist, prop_dist, dataset_info = get_generator(
        args.generators_path, dataloaders, args.device, args_gen, property_norms
    )
    ...
    for i in range(args.n_sweeps):
        ...
        save_and_sample_conditional(
            args_gen, device, model, prop_dist, dataset_info, epoch=i, id_from=0
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--classifiers_path",
        type=str,
        default="qm9/property_prediction/outputs/exp_class_alpha_pretrained",
    )
    parser.add_argument(
        "--property", type=str, default="alpha", help="'alpha', 'homo', 'lumo', 'gap', 'mu', 'Cv'"
    )
    parser.add_argument(
        "--no-cuda", action="store_true", default=False, help="enables CUDA training"
    )
    parser.add_argument("--debug_break", type=eval, default=False, help="break point or not")
    parser.add_argument("--log_interval", type=int, default=5, help="break point or not")
    parser.add_argument("--batch_size", type=int, default=1, help="break point or not")
    parser.add_argument("--iterations", type=int, default=20, help="break point or not")
    parser.add_argument(
        "--task", type=str, default="qualitative", help="naive, edm, qm9_second_half, qualitative"
    )
    parser.add_argument(
        "--n_sweeps",
        type=int,
