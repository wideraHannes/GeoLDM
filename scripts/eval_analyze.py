...
from src.geoldm.qm9 import visualizer as qm9_visualizer
import src.geoldm.qm9.losses as losses

try:
    from src.geoldm.qm9 import rdkit_functions
except ModuleNotFoundError:
...
def main():
    ...
    parser.add_argument("--batch_size_gen", type=int, default=100, help="Specify model path")
    parser.add_argument(
        "--save_to_xyz", type=eval, default=False, help="Should save samples to xyz files."
    )
    ...
    val_nll = test(
        args, generative_model, nodes_dist, device, dtype, dataloaders[val_name], partition="Val"
    )
    ...
    with open(join(eval_args.model_path, "eval_log.txt"), "w") as f:
        print(f"Overview: val nll {val_nll} test nll {test_nll}", stability_dict, file=f)
