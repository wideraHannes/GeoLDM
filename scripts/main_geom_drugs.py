...
from src.geoldm.qm9.utils import prepare_context, compute_mean_mad
import train_test
...
parser.add_argument(
    "--train_diffusion", action="store_true", help="Train second stage LatentDiffusionModel model"
)
parser.add_argument("--ae_path", type=str, default=None, help="Specify first stage model path")
parser.add_argument(
    "--trainable_ae", action="store_true", help="Train first stage AutoEncoder model"
)
...
parser.add_argument(
    "--diffusion_noise_schedule", type=str, default="polynomial_2", help="learned, cosine"
)
...
parser.add_argument(
    "--sin_embedding", type=eval, default=False, help="whether using or not the sin embedding"
)
# <-- EGNN args
...
parser.add_argument(
    "--n_stability_samples", type=int, default=20, help="Number of samples to compute the stability"
)
parser.add_argument(
    "--normalize_factors",
    type=eval,
...
parser.add_argument(
    "--normalization_factor", type=float, default=100, help="Normalize the sum aggregation of EGNN"
)
parser.add_argument(
    "--aggregation_method",
    type=str,
...
parser.add_argument(
    "--filter_molecule_size", type=int, default=None, help="Only use molecules below this size."
)
parser.add_argument(
    "--sequential",
    action="store_true",
...
split_data = build_geom_dataset.load_split_data(
    data_file, val_proportion=0.1, test_proportion=0.1, filter_size=args.filter_molecule_size
)
...
for key, data_list in zip(["train", "val", "test"], split_data):
    ...
    dataloaders[key] = build_geom_dataset.GeomDrugsDataLoader(
        sequential=args.sequential, dataset=dataset, batch_size=args.batch_size, shuffle=shuffle
    )
del split_data
        ...
        if epoch % args.test_epochs == 0:
            ...
            if nll_val < best_nll_val:
                ...
                if args.save_model:
                    ...
                    if args.ema_decay > 0:
                        utils.save_model(
                            model_ema, "outputs/%s/generative_model_ema.npy" % args.exp_name
                        )
            ...
            if args.save_model:
                ...
                utils.save_model(
                    model, "outputs/%s/generative_model_%d.npy" % (args.exp_name, epoch)
                )
                if args.ema_decay > 0:
                    utils.save_model(
                        model_ema, "outputs/%s/generative_model_ema_%d.npy" % (args.exp_name, epoch)
                    )
