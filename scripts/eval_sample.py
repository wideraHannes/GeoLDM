...
def save_and_sample_chain(
    args, eval_args, device, flow, n_tries, n_nodes, dataset_info, id_from=0, num_chains=100
):
...
def main():
    ...
    parser.add_argument(
        "--n_tries", type=int, default=10, help="N tries to find stable molecule for gif animation"
    )
    parser.add_argument(
        "--n_nodes", type=int, default=19, help="number of atoms in molecule for gif animation"
    )
    ...
    sample_different_sizes_and_save(
        args, eval_args, device, flow, nodes_dist, dataset_info=dataset_info, n_samples=30
    )
    ...
    vis.visualize(
        join(eval_args.model_path, "eval/molecules/"), dataset_info, max_num=100, spheres_3d=True
    )
