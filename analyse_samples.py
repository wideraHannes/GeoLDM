...
def analyze_molecules(
    ...
    # Extract valid SMILES from DataFrame (excluding None values)
    valid_mask = df["smiles"].notna()
    valid_smiles = df.loc[valid_mask, "smiles"].tolist()

    # Initialize fs_score column with None values
    df["fs_score"] = None

    print("adding fs_score")
    if valid_smiles:
        # Predict scores for valid SMILES
        scores = scorer.score(valid_smiles)

        # Assign scores to the corresponding rows in the DataFrame
        df.loc[valid_mask, "fs_score"] = scores
    else:
        print("No valid SMILES found in the dataset")
...
def print_molecule_statistics(df: pd.DataFrame) -> None:
    """
    Print statistics about molecule properties and save summary to CSV.
    ...
    # Create summary statistics, handling NaN values
    summary = {
        "metric": ["mean", "median", "min", "max", "count"],
        "sa_score": [
            df["sa_score"].mean(skipna=True),
            df["sa_score"].median(skipna=True),
            df["sa_score"].min(skipna=True),
            df["sa_score"].max(skipna=True),
            df["sa_score"].count(),
        ],
        "qed": [
            df["qed"].mean(skipna=True),
            df["qed"].median(skipna=True),
            df["qed"].min(skipna=True),
            df["qed"].max(skipna=True),
            df["qed"].count(),
        ],
        "fs_score": [
            df["fs_score"].mean(skipna=True),
            df["fs_score"].median(skipna=True),
            df["fs_score"].min(skipna=True),
            df["fs_score"].max(skipna=True),
            df["fs_score"].count(),
        ],
    }
    ...
    summary["success_rate"] = [
        ...
        None,
        None,
        f"{total_count - error_count}/{total_count}",
    ]

    # Convert to DataFrame
    summary_df = pd.DataFrame(summary)

    summary_path = os.path.join(ANALYSIS_DIR, "molecule_analysis_summary.csv")
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default=ANALYSIS_DIR / "molecule_analysis.csv",
