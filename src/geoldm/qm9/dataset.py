...
from src.geoldm.qm9.data.args import init_argparse
from src.geoldm.qm9.data.collate import PreprocessQM9
from src.geoldm.qm9.data.utils import initialize_datasets


def get_dataset(args, _):
    """
    Get dataset for CrossDockedPoseDataset.
    This function will be used when the dataset is "crossdock_pocket10"
    """
...
def retrieve_dataloaders(cfg):
    if cfg.dataset == "crossdock_pocket10":
        ...
        original_get_dataset_info = datasets_config.get_dataset_info
        datasets_config.get_dataset_info = (
            lambda name, rm_h: crossdock_pocket10
            if name == "crossdock_pocket10"
            else original_get_dataset_info(name, rm_h)
        )

        # Get dataloaders for CrossDockedPoseDataset
        dataloaders = get_dataloaders(
            root_dir=cfg.datadir,
            batch_size=cfg.batch_size,
            num_workers=0,  # Force single-process loading for ESM compatibility
        )
        # Rename 'valid' to 'val' to match expected naming convention
        if "valid" in dataloaders and "val" not in dataloaders:
            dataloaders["val"] = dataloaders["valid"]
        ...
        split_data = build_geom_dataset.load_split_data(
            data_file, val_proportion=0.1, test_proportion=0.1, filter_size=cfg.filter_molecule_size
        )
        transform = build_geom_dataset.GeomDrugsTransform(
            dataset_info, cfg.include_charges, cfg.device, cfg.sequential
        )
        dataloaders = {}
