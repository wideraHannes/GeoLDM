...
from src.geoldm.qm9.property_prediction.models_property import EGNN, Naive, NumNodes
import torch
from torch import nn, optim
import argparse
from src.geoldm.qm9.property_prediction import prop_utils
import json
from src.geoldm.qm9 import dataset, utils
import pickle
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exp_name", type=str, default="debug", metavar="N", help="experiment_name"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        "--outf", type=str, default="outputs", metavar="N", help="folder to output vae"
    )
    parser.add_argument("--lr", type=float, default=1e-3, metavar="N", help="learning rate")
    parser.add_argument("--nf", type=int, default=128, metavar="N", help="learning rate")
    parser.add_argument(
        "--attention", type=int, default=1, metavar="N", help="attention in the ae model"
    )
    parser.add_argument(
        "--n_layers", type=int, default=7, metavar="N", help="number of layers for the autoencoder"
    )
    parser.add_argument(
        "--property",
        type=str,
    ...
    parser.add_argument(
        "--dataset", type=str, default="qm9_first_half", metavar="N", help="qm9_first_half"
    )
    parser.add_argument(
        "--datadir", type=str, default="../../qm9/temp", metavar="N", help="qm9_first_half"
    )
    parser.add_argument("--remove_h", action="store_true")
    parser.add_argument(
        "--include_charges", type=eval, default=True, help="include atom charge or not"
    )
    parser.add_argument("--node_attr", type=int, default=0, metavar="N", help="node_attr or not")
    parser.add_argument(
        "--weight_decay", type=float, default=1e-16, metavar="N", help="weight decay"
    )
    parser.add_argument("--save_path", type=float, default=1e-16, metavar="N", help="weight decay")
    parser.add_argument(
        "--model_name", type=str, default="numnodes", metavar="N", help="egnn | naive | numnodes"
    )
    ...
    for epoch in range(0, args.epochs):
        ...
        if epoch % args.test_interval == 0:
            ...
            if val_loss < res["best_val"]:
                ...
                if args.save_model:
                    torch.save(
                        model.state_dict(), args.outf + "/" + args.exp_name + "/best_checkpoint.npy"
                    )
