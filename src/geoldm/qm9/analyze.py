...
class Histogram_cont:
    def __init__(self, num_bins=100, range=(0.0, 13.0), name="histogram", ignore_zeros=False):
        self.name = name
...
def check_stability(positions, atom_type, dataset_info, debug=False):
    ...
    for i in range(len(x)):
        for j in range(i + 1, len(x)):
            ...
            if (
                dataset_info["name"] == "crossdock_pocket10"
                or dataset_info["name"] == "qm9"
...
def main_check_stability(remove_h: bool, batch_size=32):
    ...
    def test_validity_for(dataloader):
        ...
        for [positions, atom_types] in dataloader:
            ...
            print(
                f"Stable molecules "
                f"{100.0 * count_mol_stable / count_mol_total:.2f} \t"
                f"Stable atoms: "
                f"{100.0 * count_atm_stable / count_atm_total:.2f} \t"
