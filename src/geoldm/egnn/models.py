...
from src.geoldm.egnn.egnn_new import EGNN, GNN
from src.geoldm.equivariant_diffusion.utils import remove_mean, remove_mean_with_mask
import numpy as np
...
class EGNN_encoder_QM9(nn.Module):
    def __init__(
        ...
        self.final_mlp = nn.Sequential(
            nn.Linear(hidden_nf, hidden_nf), act_fn, nn.Linear(hidden_nf, out_node_nf * 2 + 1)
        )
    ...
    def _forward(self, xh, node_mask, edge_mask, context):
        ...
        if context is not None:
            ...
            # vorher Nxd  -> Nxd+c_dim -> context erweitert die One Hot encoded dimension von atom types
