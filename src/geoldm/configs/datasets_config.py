...
# Import the crossdock dataset info
try:
    from src.geoldm.my_ext.crossdock_info import crossdock_pocket10
except ImportError:
    # If the module is not available, define a placeholder
    crossdock_pocket10 = None
...
def get_dataset_info(dataset_name, remove_h):
    if dataset_name == "qm9":
        ...
        if not remove_h:
            ...
            raise Exception("Missing config for %s without hydrogens" % dataset_name)
    elif dataset_name == "crossdock_pocket10":
        # Return the crossdock_pocket10 dataset info
        if crossdock_pocket10 is None:
            raise Exception(
                "Crossdock dataset info not available. Make sure src.geoldm.my_ext.crossdock_info is importable."
            )
        return crossdock_pocket10
    else:
